        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-12 mb-10 w-full">
          <div className="flex justify-center items-center flex-shrink-0">
             <Image
              ...
              height={200}
              src={
                "https://ui-avatars.com/api/?background=0D8ABC&color=fff&name=Inácio+Sant"
              }
              alt="Minha Foto"
              loading="lazy"
              className="z-10 h-40 w-40 md:h-52 md:w-52 rounded-full object-cover border-4 border-slate-800 shadow-2xl"
            />
          </div>
          <div className="flex flex-col items-center md:items-start text-center md:text-left max-w-2xl">
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">{title}</h1>
            {subtitle && <p className="text-lg md:text-xl text-gray-300 mb-8">{subtitle}</p>}
          </div>
        </div>
