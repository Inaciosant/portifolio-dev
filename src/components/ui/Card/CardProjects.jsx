      ...
      <aside className="bg-black text-white p-6 rounded-lg w-full font-mono shadow-xl border border-slate-800 flex flex-col h-full justify-between">
        <div>
          <div className="flex justify-between items-center mb-4 border-b border-gray-800 pb-2">
            ...
            <p className="text-xs text-gray-500">bash -- {project.name.toLowerCase().replace(/\s/g, "-")}</p>
          </div>
          
          <div className="space-y-3 text-sm">
            <div>
              <p className="text-green-400 font-bold">
                $ init project <span className="text-white">{project.name}</span>
              </p>
            ...
            <div className="flex gap-4">
                ...
                {project.repository && (
                 ...
                 {!project.repository && !project.url && (
                    <span className="text-gray-500 text-xs">// Em desenvolvimento ou privado</span>
                 )}
            </div>
